...
EXP_RATE        = 0.330 -- Multiplies exp earned from fov and quests.
...
WEAPON_SKILL_POWER  = 1.550 -- Multiplies damage dealt by Weapon Skills.
...
HARVESTING_BREAK_CHANCE = 13 -- % chance for the sickle to break during harvesting.  Set between 0 and 100.
EXCAVATION_BREAK_CHANCE = 13 -- % chance for the pickaxe to break during excavation.  Set between 0 and 100.
LOGGING_BREAK_CHANCE    = 13 -- % chance for the hatchet to break during logging.  Set between 0 and 100.
MINING_BREAK_CHANCE     = 13 -- % chance for the pickaxe to break during mining.  Set between 0 and 100.
...
-- DYNAMIS SETTINGS
    BETWEEN_2DYNA_WAIT_TIME = 0       -- Hours before player can re-enter Dynamis. Default is 1 Earthday (24 hours).
        DYNA_MIDNIGHT_RESET = false     -- if true, makes the wait time count by number of server midnights instead of full 24 hour intervals
             DYNA_LEVEL_MIN = 55       -- level min for entering in Dynamis
    TIMELESS_HOURGLASS_COST = 100000   -- refund for the timeless hourglass for Dynamis.
...
STONESKIN_CAP = 650 -- soft cap for hp absorbed by stoneskin
...
ENM_COOLDOWN = 10  -- Number of hours before a player can obtain same KI for ENMs (default: 5 days)
